...
3. **Топологическая сортировка и обнаружение циклов.** До любого `setattr` archtool выполняет DFS-топологическую сортировку графа зависимостей для определения порядка инъекции. При обнаружении цикла немедленно бросает `CircularDependencyError` с полным путём цикла.
